...
function updateRecentTracksList() {
    ...
    const recentTracksItems = recentTrackPaths
        .map(path => tracks.find(t => t.path === path))
        .filter(Boolean);

    if (recentTracksItems.length === 0) {
        const li = document.createElement('li');
    ...
    recentTracksItems.forEach(track => {
...
function applyMyPreset() {
    ...
    for (let freq of frequenciesEQ) {
        const slider = document.getElementById(eqIdByFrequency[freq]);
        const valSpan = document.getElementById(eqIdByFrequency[freq] + 'Val');
